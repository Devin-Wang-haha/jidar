...
func reconcileDB(pdb *db, create bool) (database.DB, error) {
	...
	var curJidarFileNum, curJidarOffset uint32
	err := pdb.View(func(tx database.Tx) error {
		...
		writeJidarRow := tx.Metadata().Get(writeJidarLocKeyName)
		if writeJidarRow == nil {
			str := "write cursor of jidar does not exist"
			return makeDbErr(database.ErrCorruption, str, nil)
		}

		curJidarFileNum, curJidarOffset, err = deserializeWriteRow(writeJidarRow)
		return err
	...
	// When the write cursor position found by scanning the jidar files on
	// disk is AFTER the position the metadata believes to be true, truncate
	// the files on disk to match the metadata.  This can be a fairly common
	// occurrence in unclean shutdown scenarios while the jidar files are in
	// the middle of being written.  Since the metadata isn't updated until
	// after the jidar data is written, this is effectively just a rollback
	// to the known good point before the unclean shutdown.
	wcJidar := pdb.jidar.writeCursor
	if wcJidar.curFileNum > curJidarFileNum || (wcJidar.curFileNum == curJidarFileNum &&
		wcJidar.curOffset > curJidarOffset) {

		log.Info("Jidar- Detected unclean shutdown - Repairing...")
		log.Debugf("Jidar- Metadata claims file %d, offset %d. Jidar data is "+
			"at file %d, offset %d", curJidarFileNum, curJidarOffset,
			wcJidar.curFileNum, wcJidar.curOffset)
		pdb.jidar.handleRollback(curJidarFileNum, curJidarOffset)
		log.Infof("Jidar- Database sync complete")
	}
	...
	// When the write cursor position found by scanning the jidar files on
	// disk is BEFORE the position the metadata believes to be true, return
	// a corruption error.  Since sync is called after each jidar is written
	// and before the metadata is updated, this should only happen in the
	// case of missing, deleted, or truncated jidar files, which generally
	// is not an easily recoverable scenario.  In the future, it might be
	// possible to rescan and rebuild the metadata from the jidar files,
	// however, that would need to happen with coordination from a higher
	// layer since it could invalidate other metadata.
	if wcJidar.curFileNum < curJidarFileNum || (wcJidar.curFileNum == curJidarFileNum &&
		wcJidar.curOffset < curJidarOffset) {

		str := fmt.Sprintf("Jidar- metadata claims file %d, offset %d, but "+
			"jidar data is at file %d, offset %d", curJidarFileNum,
			curJidarOffset, wcJidar.curFileNum, wcJidar.curOffset)
		log.Warnf("***Jidar- Database corruption detected***: %v", str)
		return nil, makeDbErr(database.ErrCorruption, str, nil)
	}
